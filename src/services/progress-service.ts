...
export function applyScenarioResult(
  ...
  return {
    ...
    stats: {
      money: stats.money + (payload.statChanges.money ?? 0),
      savings: stats.savings + (payload.statChanges.savings ?? 0),
      stress: Math.max(0, stats.stress + (payload.statChanges.stress ?? 0)),
      freedom: stats.freedom + (payload.statChanges.freedom ?? 0)
    }
  };
}
